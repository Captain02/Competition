	...
	<body class="bg-common stickey-menu">
		...
		<div class="main-content">
			...
			<div class="content-head content-head-section">
				...
				<form action="${APP_PATH}/admin/myHolidayTask/myHolidayTask" class="serach-form" method="get">
										...
										<tbody>
											<c:forEach items="${pageInfo.list}" var="holiday">
												<tr>
													...
													<c:if test="${herfPage==2}">
														<td>
															<fmt:formatDate value="${holiday.startExaminationTime}" pattern="yyyy-mm-dd hh:dd:ss" />
														</td>
													</c:if>
													<c:if test="${herfPage==2||herfPage==3}">
														<td>
															<fmt:formatDate value="${holiday.examinationTime}" pattern="yyyy-mm-dd hh:dd:ss" />
														</td>
													</c:if>
													<td>
														<!-- this hidden -->
														<c:if test="${herfPage==0}">
															<input id="holidayProcessinstanceid" value="${holiday.processinstanceid}" type="hidden">
															<a class="btn btn-warning btn-xs btn-assign" title="指派">
																<i class="glyphicon glyphicon-hand-right"></i>
															</a>
															<a href="${APP_PATH}/admin/myHolidayTask/examinationPage/${holiday.id}/${pageInfo.pageNum}" class="btn btn-success btn-info btn-xs"
															    title="完成">
																<i class="glyphicon glyphicon-ok-sign "></i>
															</a>
														</c:if>
														<a href="${APP_PATH}/admin/holiday/holidayNote/${holiday.id}" class="btn btn-danger btn-xs" title="查看">
						...
						<div class="container page-possiton ">
							...
							<nav aria-label="Page navigation">
								<ul class="pagination pagination-control">
									<li>
										<a href="${APP_PATH}/admin/myHolidayTask/myHolidayTask?pn=1&type=${type}&state=${state}&herfPage=${herfPage}">首页</a>
									</li>
									<c:if test="${pageInfo.hasPreviousPage}">
										<li>
											<a href="${APP_PATH}/admin/myHolidayTask/myHolidayTask?pn=${pageInfo.pageNum-1}&type=${type}&state=${state}&herfPage=${herfPage}"
											    aria-label="Previous">
									...
									<c:forEach items="${pageInfo.navigatepageNums}" var="pageNum">
										...
										<c:if test="${pageNum!=pageInfo.pageNum}">
											<li>
												<a href="${APP_PATH}/admin/myHolidayTask/myHolidayTask?pn=${pageNum}&type=${type}&state=${state}&herfPage=${herfPage}">${pageNum}</a>
											</li>
										</c:if>
									</c:forEach>
									...
									<li>
										<a href="${APP_PATH}/admin/myHolidayTask/myHolidayTask?pn=${pageInfo.pages}&type=${type}&state=${state}&herfPage=${herfPage}" aria-label="Next">
